...
class FacetsFormComponent extends Component {
  ...
  createURLParameters(formData = new FormData(this.refs.facetsForm)) {
    ...
    if (newParameters.get("filter.v.price.gte") === "")
      newParameters.delete("filter.v.price.gte");
    if (newParameters.get("filter.v.price.lte") === "")
...
class FacetInputsComponent extends Component {
  get sectionId() {
    ...
    if (!id)
      throw new Error("FacetInputs component must be a child of a section");
    return id;
  }
  ...
  prefetchPage = debounce((event) => {
    ...
    if (inputElement.checked)
      url.searchParams.delete(inputElement.name, inputElement.value);
  ...
  #updateSelectedFacetSummary() {
    ...
    const checkedInputElements = this.refs.facetInputs.filter(
      (input) => input.checked,
    );
...
class PriceFacetComponent extends Component {
  ...
  connectedCallback() {
    ...
    this.moneyFormat = this.#extractMoneyPlaceholder(
  ...
  #adjustToValidValues(input) {
    ...
    const min = this.#parseDisplayValue(
      input.getAttribute("data-min") ?? "0",
      currency,
    );
    const max = this.#parseDisplayValue(
...
class FacetClearComponent extends Component {
  ...
  clearFilter(event) {
    ...
    const container = event.target.closest(
      "facet-inputs-component, price-facet-component",
    );
    container
...
class FacetStatusComponent extends Component {
  ...
  updateListSummary(checkedInputElements) {
    ...
    this.getAttribute("facet-type") === "swatches"
      ? this.#updateSwatchSummary(
          checkedInputElements,
          checkedInputElementsCount,
        )
      : this.#updateBubbleSummary(
          checkedInputElements,
          checkedInputElementsCount,
        );
  }
  ...
  updatePriceSummary(minInput, maxInput) {
    ...
    const maxInputNum = this.#parseCents(
      maxInputValue,
      facetStatus.dataset.rangeMax,
      currency,
    );
